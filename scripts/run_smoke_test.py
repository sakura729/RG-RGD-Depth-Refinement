#!/usr/bin/env python
"""End-to-end smoke test for the RG-RGD release.

The test creates tiny synthetic VOID-style and RGB-D/IMU-style datasets, then
runs one reduced training epoch for both released entry points. It verifies
imports, dataset loading, forward passes, losses, backpropagation, validation,
and checkpoint saving. It is not intended to reproduce paper metrics.
"""
...
def write_rgbd_imu_dataset(root: Path) -> Path:
    ...
    h, w = 96, 96
    yy, xx = np.mgrid[0:h, 0:w]
    for k, ts in enumerate([0, 50000, 100000, 150000]):
        rgb = np.zeros((h, w, 3), dtype=np.uint8)
        rgb[..., 0] = (xx + k * 8) % 255
        rgb[..., 1] = (yy + 40) % 255
        rgb[..., 2] = 80 + k * 20
    ...
    with open(seq / "imu.csv", "w", encoding="utf-8") as f:
        ...
        for ts in range(0, 150001, 10000):
            f.write(f"{ts},0,0,9.81,0.001,0.002,0.003\n")

    return seq_root


def run(cmd: list[str], cwd: Path) -> None:
    print("\n[smoke] " + " ".join(cmd), flush=True)
    subprocess.run(cmd, cwd=str(cwd), check=True)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--work_dir", type=Path, default=Path(".smoke_test_runs"))
    ...
    expected = [
        runs / "void_smoke" / "rgrgd_void_best.pth",
        runs / "selfsup_smoke" / "ckpt_best.pth",
        runs / "selfsup_smoke" / "ckpt_last.pth",
    ]
    missing = [str(p) for p in expected if not p.is_file()]
    if missing:
        raise RuntimeError("Smoke test finished but checkpoint files are missing: " + ", ".join(missing))

    print("\n[smoke] PASS")
    for p in expected:
        print(f"[smoke] {p} ({p.stat().st_size} bytes)")
